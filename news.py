...
# URL of the RSS feed
rss_url = "http://telegraphnepal.com/feed/"  # Replace with the URL of your RSS feed
...
def fetch_data():
    try:
        ...
        if response.status_code == 200:
            ...
            # Clear existing data in the MongoDB Atlas collection
            collection.delete_many({})

            # Iterate through <item> elements and directly insert them into the collection
            for item_elem in root.findall('.//item'):
                ...
                # Create a dictionary for each item and insert it directly into the collection
                item_data = {
                    'title': title,
                    'link': link,
...
def get_items():
    # Retrieve data from the MongoDB Atlas collection
